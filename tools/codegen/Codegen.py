...
class Codegen:
    ...
    def read_input_files(self):
        ignore_paths = [
            ...
            os.path.join(self.state.src_dir, "core")
        ]
        ...
        for root, dirs, files in os.walk(self.state.src_dir):
            for file in files:
                ...
                if file.endswith('.hpp') and all([ignore_path not in root for ignore_path in ignore_paths]):
                    self.header_files.append((filepath, last_modified))
                elif file.endswith('.cpp') and all([ignore_path not in root for ignore_path in ignore_paths]):
