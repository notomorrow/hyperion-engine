...
void Editor::InitGame()
{
    ...
    { // init server
        ...
        m_socket_server.SetEventProc(HYP_NAME(OnServerStarted), [](Array<net::SocketProcArgument> &&args)
        {
            DebugLog(LogType::Info, "Socket server started\n");
        });

        m_socket_server.SetEventProc(HYP_NAME(OnError), [](Array<net::SocketProcArgument> &&args)
        {
            DebugLog(LogType::Error, "Socket error: %s\n",
                     args[0].Get<String>().Data());
        });

        m_socket_server.SetEventProc(HYP_NAME(OnClientConnected), [](Array<net::SocketProcArgument> &&args)
        {
            DebugLog(LogType::Info, "Socket client connected: %s\n",
                     args[0].Get<Name>().LookupString());
        });

        m_socket_server.SetEventProc(HYP_NAME(OnClientDisconnected), [](Array<net::SocketProcArgument> &&args)
        {
        ...
        m_socket_server.SetEventProc(HYP_NAME(OnClientData), [](Array<net::SocketProcArgument> &&args)
        {
    ...
    if (results["test_model"]) {
        ...
        m_scene->GetEntityManager()->AddComponent(
            env_grid_entity, EnvGridComponent{ENV_GRID_TYPE_SH});
    }
    ...
    // Add sun
    {
        ...
        m_scene->GetEntityManager()->AddComponent(
            sun_entity,
            TransformComponent{
                Transform(Vec3f(-0.1f, 0.65f, 0.1f).Normalized(),
    ...
    // Add Skybox
    {
        ...
        m_scene->GetEntityManager()->AddComponent(
            skybox_entity, TransformComponent{
                Transform(Vec3f::zero, Vec3f(10.0f),
                          Quaternion::Identity())
            });
