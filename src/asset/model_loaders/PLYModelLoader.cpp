...
static void ReadPropertyValue(ByteBuffer &buffer, PLYModelLoader::PLYModel &model, SizeType row_offset, const String &property_name, SizeType count, void *out_ptr) {
    ...
    //DebugLog(LogType::Debug, "%s offset: %llu\n", it->key.Data(), it->value.offset);
...
PLYModel PLYModelLoader::LoadModel(LoaderState &state)
{
    ...
    state.stream.ReadLines([&](const String &line, bool *stop_ptr) {
        ...
        if (split[0] == "property") {
            ...
            row_length += PLYTypeSize(property_type) / 8;
    ...
    for (auto &it : model.property_types) {
        ...
        if (IsCustomPropertyName(it.key)) {
            ...
            if (custom_data_it == model.custom_data.End()) {
                model.custom_data.Set(it.key, ByteBuffer(num_vertices * PLYTypeSize(it.value.type) / 8));
            }
        }
    }

    //DebugLog(LogType::Debug, "Header length: %u\n", model.header_length);
    //DebugLog(LogType::Debug, "Max length: %u\n", state.stream.Max());
    //DebugLog(LogType::Debug, "Buffer length: %u\n", buffer.Size());

    DebugLog(LogType::Debug, "num_vertices = %u\n", num_vertices);
