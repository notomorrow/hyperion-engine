...
public:
    ...
    {
        AssertDebugMsg(id.GetTypeID() == IDType::type_id_static, "ResourceTracker does not support subclasses of the element type!");
...
template <class IDType, class ElementType>
class PolymorphicResourceTracker
{
public:
    enum ResourceTrackState : uint8
    {
    ...
    PolymorphicResourceTracker(const PolymorphicResourceTracker& other) = delete;
    PolymorphicResourceTracker& operator=(const PolymorphicResourceTracker& other) = delete;

    PolymorphicResourceTracker(PolymorphicResourceTracker&& other) noexcept = delete; // for now, just keep these deleted until implemented
    PolymorphicResourceTracker& operator=(PolymorphicResourceTracker&& other) noexcept = delete;
    ...
    // @NOTE: Cannot use Reserve() because we don't know what types will be pushed causing creation
    // of new containers. We'd have to construct every subclass impl.
    // Need a better solution. Could maybe pass in all the Type IDs we will push? But that sucks.
    // HYP_FORCE_INLINE void Reserve(SizeType capacity)
    // {
    ...
    //         m_subclass_impls[bit_index].Get().Reserve(capacity);
    //     }
    // }
    ...
    typename MapType::Iterator Track(IDType id, const ElementType& element, ResourceTrackState* out_track_state = nullptr)
    {
        ...
        {
            return m_impl.Track(id, element, out_track_state);
        }
        ...
        return m_subclass_impls[subclass_index].Get().Track(id, element, out_track_state);
    }
    ...
    typename MapType::Iterator Track(IDType id, ElementType&& element, ResourceTrackState* out_track_state = nullptr)
    {
        ...
        if (type_id == m_impl.type_id)
        {
            return m_impl.Track(id, std::move(element), out_track_state);
        }
        ...
        return m_subclass_impls[subclass_index].Get().Track(id, std::move(element), out_track_state);
    }
...
protected:
    struct Impl final
    {
        ...
        HYP_FORCE_INLINE bool HasElement(IDType id) const
        {
            AssertDebugMsg(id.GetTypeID() == type_id, "PolymorphicResourceTracker typeid mismatch");
        ...
        HYP_FORCE_INLINE void Reserve(SizeType capacity)
        {
            element_map.Reserve(capacity);
        }

        typename MapType::Iterator Track(IDType id, const ElementType& element, ResourceTrackState* out_track_state = nullptr)
        {
            AssertDebugMsg(id.GetTypeID() == type_id, "PolymorphicResourceTracker type id mismatch");

            typename MapType::Iterator iter = element_map.End();

            AssertThrowMsg(!next.Test(id.ToIndex()), "ID #%u already marked to be added for this iteration!", id.Value());

            if (HasElement(id))
            {
                iter = element_map.FindAs(id);
            }
            ...
            else
            {
                ...
                iter = element_map.Insert(id, element).first;
        ...
        typename MapType::Iterator Track(IDType id, ElementType&& element, ResourceTrackState* out_track_state = nullptr)
        {
            ...
            else
            {
                ...
                iter = element_map.Insert(id, std::move(element)).first;
        ...
        void GetRemoved(Array<ElementType, AllocatorType>& out, bool include_changed) const
        {
            ...
            while ((first_set_bit_index = removed_bits.FirstSetBitIndex()) != Bitset::not_found)
            {
                ...
                AssertThrow(it != element_map.End());
        ...
        ElementType* GetElement(IDType id)
        {
            const auto it = element_map.Find(id);
        ...
        void Advance(AdvanceAction action)
        {
            ...
            { // Remove proxies for removed bits
                for (Bitset::BitIndex index : GetRemoved())
                {
                    const IDType id = IDType(IDBase { type_id, uint32(index + 1) });
...
using RenderProxyTracker = PolymorphicResourceTracker<ID<Entity>, RenderProxy>;
