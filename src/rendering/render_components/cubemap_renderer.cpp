...
CubemapRendererPass::CubemapRendererPass()
    : FullScreenPass(),
      m_max_distance(100.0f)
{
}

CubemapRendererPass::~CubemapRendererPass() = default;

void CubemapRendererPass::CreateShader(Engine *engine)
{
    m_shader = engine->resources.shaders.Add(std::make_unique<Shader>(
        std::vector<SubShader>{
            SubShader{ ShaderModule::Type::VERTEX, {FileByteReader(FileSystem::Join(engine->assets.GetBasePath(), "vkshaders/forward_vert.spv")).Read()} },
            SubShader{ ShaderModule::Type::FRAGMENT, {FileByteReader(FileSystem::Join(engine->assets.GetBasePath(), "vkshaders/forward_frag.spv")).Read()} }
        }
    ));
...
void CubemapRendererPass::SetParentScene(Scene::ID id)
{
