...
using EntityBatchIndex = SizeType;
...
struct ShaderGlobals
{
    ...
    bool PushEntityToBatch(EntityBatchIndex batch_index, HandleID<Entity> entity_id)
    {
        ...
        //AssertThrowMsg(
        //    batch.num_entities < EntityInstanceBatch::max_entities_per_instance_batch,
        //    "Batch capacity exceeded"
        //);

        if (batch.num_entities >= EntityInstanceBatch::max_entities_per_instance_batch) {
            return false;
        }

        const UInt32 id_index = batch.num_entities++;
        batch.ids[id_index] = UInt32(entity_id.Value());
        entity_instance_batches.MarkDirty(batch_index);
    ...
    {
        AssertThrow(batch_index < max_entity_instance_batches);
        AssertThrow(id_index < EntityInstanceBatch::max_entities_per_instance_batch);
