...
void DeferredPipeline::RenderOpaqueBuckets(Renderer *renderer, Camera *cam, Framebuffer2D *fbo)
{
    ...
    renderer->RenderBucket(cam, renderer->GetBucket(Spatial::Bucket::RB_SKY));
    CoreEngine::GetInstance()->Enable(CoreEngine::GLEnums::CULL_FACE);

    renderer->RenderBucket(cam, renderer->GetBucket(Spatial::Bucket::RB_OPAQUE));
...
void DeferredPipeline::Render(Renderer *renderer, Camera *cam, Framebuffer2D *fbo)
{
    ...
    */
    ...
    renderer->RenderBucket(cam, renderer->GetBucket(Spatial::Bucket::RB_TRANSPARENT));
    renderer->RenderBucket(cam, renderer->GetBucket(Spatial::Bucket::RB_PARTICLE));
