...
#define HYPERION_VK_CHECK(vk_result) \
    do { \
        if ((vk_result) != VK_SUCCESS) \
            return Result(Result::RENDERER_ERR, #vk_result " != VK_SUCCESS"); \
    } while (0)

#define HYPERION_VK_PASS_ERRORS(vk_result, out_result) \
    do { \
        if ((vk_result) != VK_SUCCESS) (out_result) = Result(Result::RENDERER_ERR, #vk_result " != VK_SUCCESS"); \
    } while (0)

#define HYPERION_VK_CHECK_MSG(vk_result, msg) \
    do { \
        if ((vk_result) != VK_SUCCESS) \
            return Result(Result::RENDERER_ERR, msg ":\t" #vk_result " != VK_SUCCESS"); \
    } while (0)
