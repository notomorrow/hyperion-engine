...
void PostProcessingEffect::RenderEffect(IFrame *frame, uint32 slot)
{
    struct alignas(128)
    {
...
void PostProcessing::CreateUniformBuffer()
{
    ...
    HYPERION_ASSERT_RESULT(m_uniform_buffer->Create(sizeof(post_processing_uniforms)));
...
void PostProcessing::RenderPre(IFrame *frame) const
{
    Threads::AssertOnThread(g_render_thread);
...
void PostProcessing::RenderPost(IFrame *frame) const
{
    Threads::AssertOnThread(g_render_thread);
