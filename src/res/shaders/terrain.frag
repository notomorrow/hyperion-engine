...
void main() 
{
...
#if NORMAL_MAP
  vec3 normalsTexture;
  normalsTexture.xy = (2.0 * (vec2(1.0) - texture2D(terrainTexture0Normal, v_texcoord0 * terrainTexture0Scale).rg) - 1.0);
  ...
  normalsTexture;
  normalsTexture.xy = (2.0 * (vec2(1.0) - texture2D(slopeTextureNormal, v_texcoord0 * slopeTextureScale).rg) - 1.0);
  normalsTexture.z = sqrt(1.0 - dot(normalsTexture.xy, normalsTexture.xy));
