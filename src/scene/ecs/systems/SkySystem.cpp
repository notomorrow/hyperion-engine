...
void SkySystem::OnEntityAdded(const Handle<Entity> &entity)
{
    ...
    if (!sky_component.render_subsystem) {
        sky_component.render_subsystem = GetEntityManager().GetScene()->GetRenderResources().GetEnvironment()->AddRenderSubsystem<SkydomeRenderer>(Name::Unique("sky_renderer"));
    }
...
void SkySystem::OnEntityRemoved(ID<Entity> entity)
{
    ...
    if (sky_component.render_subsystem) {
        GetEntityManager().GetScene()->GetRenderResources().GetEnvironment()->RemoveRenderSubsystem<SkydomeRenderer>(sky_component.render_subsystem->GetName());
