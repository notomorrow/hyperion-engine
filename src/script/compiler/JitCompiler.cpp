...
std::unique_ptr<Buildable> JitCompiler::BuildArgumentsStart(
    AstVisitor *visitor,
    Module *mod,
...
std::unique_ptr<Buildable> JitCompiler::BuildArgumentsEnd(
    AstVisitor *visitor,
    Module *mod,
    size_t nargs)
{
    ...
    return JitCompiler::PopStack(visitor, nargs);
}

std::unique_ptr<Buildable> JitCompiler::BuildCall(
    AstVisitor *visitor,
    Module *mod,
...
std::unique_ptr<Buildable> JitCompiler::BuildMethodCall(
    AstVisitor *visitor,
    Module *mod,
    ...
    const std::vector<std::shared_ptr<AstArgument>> &args)
{
    ...
    // pop arguments from stack
    chunk->Append(JitCompiler::PopStack(visitor, nargs));

    return std::move(chunk);
}

std::unique_ptr<Buildable> JitCompiler::LoadMemberFromHash(AstVisitor *visitor, Module *mod, uint32_t hash)
{
    uint8_t rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
...
std::unique_ptr<Buildable> JitCompiler::StoreMemberFromHash(AstVisitor *visitor, Module *mod, uint32_t hash)
{
    uint8_t rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
...
std::unique_ptr<Buildable> JitCompiler::LoadMemberAtIndex(AstVisitor *visitor, Module *mod, int dm_index)
{
    uint8_t rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
...
std::unique_ptr<Buildable> JitCompiler::StoreMemberAtIndex(AstVisitor *visitor, Module *mod, int dm_index)
{
    uint8_t rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
...
std::unique_ptr<Buildable> JitCompiler::CreateConditional(
    AstVisitor *visitor,
    Module *mod,
    AstStatement *cond,
...
std::unique_ptr<Buildable> JitCompiler::LoadLeftThenRight(AstVisitor *visitor, Module *mod, JitCompiler::ExprInfo info)
{
    std::unique_ptr<BytecodeChunk> chunk = BytecodeUtil::Make<BytecodeChunk>();
...
std::unique_ptr<Buildable> JitCompiler::LoadRightThenLeft(AstVisitor *visitor, Module *mod, JitCompiler::ExprInfo info)
{
    std::unique_ptr<BytecodeChunk> chunk = BytecodeUtil::Make<BytecodeChunk>();
...
std::unique_ptr<Buildable> JitCompiler::LoadLeftAndStore(
    AstVisitor *visitor,
    Module *mod,
    JitCompiler::ExprInfo info)
{
    std::unique_ptr<BytecodeChunk> chunk = BytecodeUtil::Make<BytecodeChunk>();
...
std::unique_ptr<Buildable> JitCompiler::BuildBinOp(uint8_t opcode,
    AstVisitor *visitor,
    Module *mod,
    JitCompiler::ExprInfo info)
{
    ...
    if (left_as_binop == nullptr && right_as_binop != nullptr) {
        ...
        chunk->Append(JitCompiler::LoadRightThenLeft(visitor, mod, info));
        rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
        ...
        if (info.left->MayHaveSideEffects()) {
            chunk->Append(JitCompiler::LoadLeftAndStore(visitor, mod, info));
            rp = visitor->GetCompilationUnit()->GetInstructionStream().GetCurrentRegister();
        ...
        chunk->Append(JitCompiler::LoadLeftThenRight(visitor, mod, info));
...
std::unique_ptr<Buildable> JitCompiler::PopStack(AstVisitor *visitor, int amt)
{
    if (amt == 1) {
        auto instr_pop = BytecodeUtil::Make<RawOperation<>>();
        instr_pop->opcode = POP;
...
JitCompiler::JitCompiler(AstIterator *ast_iterator, CompilationUnit *compilation_unit)
    : AstVisitor(ast_iterator, compilation_unit), m_page(std::make_unique<Page>())
{
}
...
std::unique_ptr<BytecodeChunk> JitCompiler::Compile()
{
