...
{
    Variant<VMString, VMObject, Any> m_variant;
...
public:
    ...
    template <class T>
    HYP_FORCE_INLINE void Assign(const T& value)
    {
        if constexpr (IS_INLINE_TYPE(T))
        {
            m_variant.Set(value);
    ...
    template <typename T>
    HYP_FORCE_INLINE void Assign(T&& value)
    {
        if constexpr (IS_INLINE_TYPE(T))
        {
...
public:
    ...
    HYP_FORCE_INLINE int& GetFlags()
    {
        return m_flags;
    }

    HYP_FORCE_INLINE int GetFlags() const
    {
        return m_flags;
    }
    ...
    template <class T>
    HYP_FORCE_INLINE bool TypeCompatible() const
    {
        return m_impl.Is<T>();
    }
