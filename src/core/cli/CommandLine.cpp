...
CommandLineArguments CommandLineArguments::Merge(const CommandLineArgumentDefinitions &definitions, const CommandLineArguments &a, const CommandLineArguments &b)
{
    ...
    for (const Pair<String, CommandLineArgumentValue> &element : b) {
        ...
        AppendCommandLineArgumentValue(result.values, element.first, element.second, allow_multiple);
    }
...
Result<CommandLineArguments> CommandLineParser::Parse(const String &command, const Array<String> &args) const
{
    ...
    for (SizeType i = 0; i < args.Size(); i++) {
        ...
        if (it->type == CommandLineArgumentType::BOOLEAN) {
            AppendCommandLineArgumentValue(result.values, arg, CommandLineArgumentValue { true }, allow_multiple);
