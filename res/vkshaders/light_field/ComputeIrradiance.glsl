...
int GetLocalEnvProbeIndex(vec3 world_position, vec3 grid_aabb_extent, ivec3 grid_size, out ivec3 unit_diff)
{
    const vec3 size_of_probe = grid_aabb_extent / vec3(grid_size);
    const ivec3 position_units = ivec3(world_position / size_of_probe + (vec3(grid_size) * 0.5));
    ...
    int probe_index_at_point = (int(unit_diff.x) * grid_size.y * grid_size.z)
        + (int(unit_diff.y) * grid_size.z)
...
{
    const vec3 size_of_probe = grid_aabb_extent / vec3(grid_size);
    const vec3 grid_aabb_min = grid_center - (grid_aabb_extent * 0.5);

    ivec3 base_probe_coord;
    const int base_probe_index_indirect = GetLocalEnvProbeIndex(world_position, grid_aabb_extent, grid_size, base_probe_coord);
    
    if (base_probe_index_indirect < 1 || base_probe_index_indirect >= HYP_MAX_BOUND_LIGHT_FIELD_PROBES) {
        return vec3(0.0);
    }
    ...
    const vec3 base_probe_position_world = grid_aabb_min + (vec3(base_probe_coord) * size_of_probe);

    vec3 alpha = clamp((world_position - base_probe_position_world) / size_of_probe + 0.5, vec3(0.0), vec3(1.0));
    ...
    for (int i = 0; i < 8; i++) {
        ...
        //ivec3 neighbor_grid_coord = clamp(base_probe_coord + offset, ivec3(0), ivec3(grid_size) - 1);

        const vec3 probe_position_world =  base_probe_position_world + (vec3(offset) * size_of_probe);
        

        ivec3 neighbor_probe_coord;
        const int neighbor_probe_index_indirect = GetLocalEnvProbeIndex(probe_position_world, grid_aabb_extent, grid_size, neighbor_probe_coord);
    
        if (neighbor_probe_index_indirect < 1 || neighbor_probe_index_indirect >= HYP_MAX_BOUND_LIGHT_FIELD_PROBES) {
            continue;
        }
...
vec3 ComputeLightFieldProbeIrradiance_Old(vec3 world_position, vec3 N, vec3 V, vec3 grid_center, vec3 grid_aabb_extent, ivec3 grid_size)
{
    ...
    const vec3 base_probe_position_world = env_probes[base_probe_index].world_position.xyz; // vec3(base_grid_coord) * size_of_probe + grid_aabb_min;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        const vec3 probe_position_world = env_probes[neighbor_probe_index].world_position.xyz;//vec3(neighbor_grid_coord) * size_of_probe + grid_aabb_min;
        const vec3 probe_to_point = world_position - probe_position_world; // + (N + 3.0 * V) * 0.1;
        const vec3 dir = normalize(probe_to_point);
        ...
        vec2 irradiance_uv = TextureCoordFromDirection(irradiance_dir, neighbor_probe_index, probe_grid_dimensions, grid_size);
        vec3 irradiance = Texture2DLod(sampler_linear, light_field_color_buffer, irradiance_uv, 0.0).rgb;
       ...
       // weight = 1.0 / 8.0;
        //total_irradiance += vec3(dir) * weight;//irradiance * weight;
        //total_irradiance += vec3(neighbor_grid_coord) * 0.2 * weight;

        //total_irradiance += UINT_TO_VEC4(neighbor_probe_index).xyz * weight;
        total_irradiance += irradiance * weight;
        total_weight += weight;
    }
