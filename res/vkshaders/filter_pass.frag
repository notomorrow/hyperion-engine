...
layout(location=8) in uint v_filter_frame_src;
...
void main() {
    gbuffer_albedo = vec4(1.0, 0.5, 0.0, 1.0);//vec4(texture(tex, reflection_vector).rgb, 1.0);//vec4(textureLod(tex, vec2(v_texcoord0.x, 1.0 - v_texcoord0.y), 7).rgb, 1.0);
