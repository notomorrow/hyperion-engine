...
layout(set = 0, binding = 1, OUTPUT_FORMAT) uniform readonly image2D ssr_sample_image;
...
layout(set = 0, binding = 7) uniform texture2D ssr_radius;
    ...
    layout(set = 0, binding = 3, OUTPUT_FORMAT) uniform writeonly image2D ssr_blur;
    #define HYP_SSR_PREV_IMAGE ssr_sample_image
    #define HYP_SSR_PREV_TEXTURE ssr_sample
#elif defined(HYP_SSR_BLUR_VERTICAL)
    layout(set = 0, binding = 3, OUTPUT_FORMAT) uniform readonly image2D ssr_blur_hor_image;
    layout(set = 0, binding = 4, OUTPUT_FORMAT) uniform writeonly image2D ssr_blur;
...
)
{
    for (int i = 1; i < GAUSS_TABLE_SIZE; i++) {
        ...
        ivec2 tc = ivec2(texcoord * vec2(ssr_params.dimension.xy)) + blur_step * direction;//ivec2(blur_step * direction);
...
void main(void)
{
	...
	const vec2 texcoord = (vec2(coord) + 0.5) / vec2(ssr_params.dimension.xy);
    ...
    reflection_sample = imageLoad(HYP_SSR_PREV_IMAGE, ivec2(coord)) * divisor;
    accum_radius = imageLoad(ssr_radius_image, ivec2(coord)).r * divisor;
    ...
    accum_radius  = imageLoad(ssr_radius_image, ivec2(coord)).r;
    ...
    for (int x = -1; x <= 1; ++x) {
        ...
        vec2 offset_texcoord = offset_coord * blur_amount / vec2(ssr_params.dimension.xy);
...
// #endif
    ...
    imageStore(ssr_blur, ivec2(coord), clamp(reflection_sample, vec4(0.0), vec4(1.0)));
}
