...
void main() {
  ...
  //v_tangent = (normalMatrix * vec4(a_tangent, 0.0)).xyz;
  //v_bitangent = (normalMatrix * vec4(a_bitangent, 0.0)).xyz;
#endif
	...
	v_tangentWS = normalMatrix * vec4(v_tangent, 0.0); //vec4(v_tangent, 0.0);//
	v_bitangentWS = normalMatrix * vec4(v_bitangent, 0.0); //vec4(v_bitangent, 0.0);//
