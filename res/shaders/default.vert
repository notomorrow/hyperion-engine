...
out vec4 v_tangentWS;
out vec4 v_bitangentWS;
out mat3 v_tbn;
...
void main() {
  ...
  normalMatrix = transpose(inverse(u_modelMatrix));
	...
	//v_tangent = calculate_tangent(a_normal.xyz);
	//v_bitangent = cross(a_normal.xyz, v_tangent);
    //v_tangentWS = normalMatrix * vec4(a_tangent, 0.0);
    //v_bitangentWS = normalMatrix * vec4(a_bitangent, 0.0);
  ...
  v_tangentWS = normalMatrix * vec4(v_tangent, 0.0);
  v_bitangentWS = normalMatrix * vec4(v_bitangent, 0.0);
