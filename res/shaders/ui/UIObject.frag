...
void main()
{
    ...
    if (object.material_index != ~0u) {
        material = materials[object.material_index];
    } else {
        material.albedo = vec4(1.0, 1.0, 1.0, 1.0);
        material.texture_usage = 0;
    }
#endif

    vec4 ui_color = material.albedo;
    ...
    if (HAS_TEXTURE(material, MATERIAL_TEXTURE_ALBEDO_map)) {
        vec4 albedo_texture = SAMPLE_TEXTURE(material, MATERIAL_TEXTURE_ALBEDO_map, v_texcoord0);
