...
layout(location = 2) in vec3 scale; // the particle's scale
layout(location = 3) in float lifespan; // opacity value
...
void main()
{
  ...
  v_position = vec4(offset + (right * scaledPosition.x) + (up * scaledPosition.y), 1.0);
