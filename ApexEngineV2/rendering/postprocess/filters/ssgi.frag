...
vec3 getPosition(vec2 uv, float depth) {
        ...
        vec3 ray = inverse(u_projMatrix) * vec4((uv.x - 0.5) *2., ((uv.y - 0.5) * 2.), 1.0, 1.0);
        ray *= depth;
        return ray.xyz;
}
...
{                                            
    ...
    for (int x = 0; x < 4; x++) {
        for (int y = 0; y < 4; y++) {
            ...
            //float sampleDepth = unpack_depth(texture2D(DepthMap, v_texcoord0+vec2(npw,nph)));
            vec3 ddiff = getPosition(v_texcoord0+vec2(npw,nph), depth);
            //ao += aoFF(ddiff,normals,npw,nph);
            gi += giFF(ddiff,normals,npw,nph);
        }
    }
